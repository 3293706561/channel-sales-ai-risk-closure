...
const roleLabels = { regional_manager: "区域经理", sales: "一线销售", sales_director: "销售总监" };
...
function layout(body) {
  ...
  return `<div class="shell"><aside class="side"><div class="brand">特渠经营管理<small>经营控制室 · 演示环境</small></div><nav class="nav">${nav.map(([id, name]) => `<button data-page="${id}" class="${state.page === id ? "active" : ""}">${name}</button>`).join("")}</nav><div class="side-foot"><b>${state.user.name}</b><br>${roleLabels[state.user.role] || state.user.role}<br><button id="logout" class="click" style="color:#fff;margin-top:10px">切换演示角色</button></div></aside><section class="content"><header class="top"><div><small>风险控制台 / 模拟数据</small></div><div><small>剧本时间 · 2026-09-02</small></div></header>${body}</section></div>`;
}
...
async function detailView() {
  ...
  app.innerHTML = layout(`<button class="click" data-page="risks">← 返回风险预警</button><div class="headline" style="margin-top:14px"><h1>${risk.title}</h1><p class="sub">${risk.summary}</p></div><section class="detail"><div><div class="panel"><div class="panel-head"><h2>${tag(risk)}　${risk.ruleCode}</h2><small>版本 ${risk.version}</small></div><h3>系统发现的信号</h3><div class="signals">${risk.signals.map(signal => `<div class="signal"><b>${signal.text}</b><small>${signal.source} · ${signal.evidenceRef} · ${formatDate(signal.observedAt)}</small></div>`).join("")}</div><div class="ai"><b>下一步建议（待业务确认）</b><p>先核验这些信号是否反映真实业务变化，再确认责任人、截止时间和需要协同的事项。</p></div><div class="actions">${actionButtons.join("") || `<span class="sub">当前角色没有可执行操作。</span>`}</div></div></div><aside><section class="panel"><h2>任务信息</h2><p><b>状态：</b>${tag(risk)}</p><p><b>责任人：</b>${task?.assignee?.name || risk.owner?.name || "—"}</p><p><b>截止：</b>${formatDate(risk.dueAt)}</p>${task ? `<p><b>任务版本：</b>${task.version}</p>` : ""}</section>${task ? `<section class="panel" style="margin-top:16px"><div class="audit-detail-head"><span>WORK LOG</span><h2>整改进度</h2></div><div class="progress-log">${task.updates.length ? task.updates.map(update => `<div><b>${update.content}</b><small>${update.author?.name || "责任人"} · ${formatDate(update.at)}</small></div>`).join("") : `<p class="sub">责任人尚未补充整改进度。</p>`}</div></section>` : ""}<section class="panel" style="margin-top:16px"><div class="audit-detail-head"><span>SERVER EVENT LOG</span><h2>处理留痕</h2></div><div class="timeline">${risk.timeline.map(event => `<div><b>${eventLabels[event.event] || event.event}</b><small>${event.actor?.name || "系统"} · ${formatDate(event.at)} · 服务端已写入</small></div>`).join("")}</div></section></aside></section>`);
  bindShell();
